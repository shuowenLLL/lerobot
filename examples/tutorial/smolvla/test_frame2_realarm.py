...
# ===== 单位解释 =====
# 当前这份脚本仍然按“模型输出 / 保存 / rerun 使用 degrees”来理解
...
def flatten_state_tensor(x: torch.Tensor | np.ndarray) -> np.ndarray:
    """
    把 [6] / [1,6] / 其他可展平形式统一转成一维 numpy 数组。
    """
...
def build_robot_dataset_features(robot):
    """
    构造 build_inference_frame / make_robot_action 所需的 dataset feature 映射
    """
...
def save_debug_artifacts(sample: dict, task: str, input_state_used, action_chunk_deg: np.ndarray) -> None:
    ...
    meta = {
        ...
        "state_source": "robot.get_observation() -> build_inference_frame(...)",
        "image_source": "dataset sample (up/side)",
    }
...
def send_action_chunk_to_real_robot(robot, dataset_features, action_chunk_tensor: torch.Tensor) -> None:
    ...
    输入应为 postprocess 后、CPU 上的 tensor。
    """
...
def main():
    ...
    try:
        ...
        # 4) 构造模型输入
        #    图片 = 数据集 sample
        #    state = 真机 raw_obs 经 build_inference_frame(...) 处理后的 state
        if SEND_TO_REAL_ROBOT:
            ...
            obs_frame = build_mixed_obs_frame_v2(
                sample=sample,
                robot=robot,
